...
# Try to import Gremlin for Neptune support
try:
    ...
    print("⚠️ Gremlin not available, using in-memory fallback only")
...
class VillageKnowledgeGraph:
    ...
    def __init__(self, neptune_endpoint: str = None, neptune_port: int = 8182):
        ...
        self.endpoint = neptune_endpoint or os.environ.get("NEPTUNE_ENDPOINT") or NEPTUNE_ENDPOINT
        self.port = neptune_port
        self.connection_string = f"wss://{self.endpoint}:{self.port}/gremlin"
        
        # For local testing without Neptune or Gremlin, use in-memory storage
        self.use_neptune = neptune_endpoint is not None and GREMLIN_AVAILABLE
        self.local_graph = {"farmers": [], "villages": [], "crops": []}
        
        if not GREMLIN_AVAILABLE:
            print("📝 Using in-memory knowledge graph (Gremlin not available)")
    
    def get_connection(self):
        """Get Gremlin connection to Neptune"""
        if not self.use_neptune or not GREMLIN_AVAILABLE:
            return None
